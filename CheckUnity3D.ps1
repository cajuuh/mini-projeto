...
elseif(!(Test-Path -Path "$dependenciesDir\$currentVersion"))
{
    ...
    # Download Unity3D editor and Android build support
    ...
    wget -OutFile "$dependenciesDir\$currentVersion\UnitySetup-Windows-Support-for-Editor-$currentVersionNumber.exe" "http://netstorage.unity3d.com/unity/$currentRevision/TargetSupportInstaller/UnitySetup-Android-Support-for-Editor-$currentVersionNumber.exe"

    # Install Unity3D editor and Android build support
    ...
    Start-Process -Wait -FilePath "$dependenciesDir\$currentVersion\UnitySetup-Android-Support-for-Editor-$currentVersionNumber.exe" -ArgumentList "/S /D=$dependenciesDir\$currentVersion\Unity"
